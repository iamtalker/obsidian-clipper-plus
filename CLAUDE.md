...
2. **버전은 바뀐 쪽만 올린다**(사용자 방침, 2026-09-25): 공유 파일이 바뀌어 두 확장 모두 달라지면 둘 다, 한쪽 전용 파일만 바뀌면 그쪽만 올린다. 두 레포 버전 번호는 달라도 된다. 올릴 때는 `manifest.json` 버전 → README "변경 이력" 맨 위에 한글로 원인+해결 기록 → 커밋 → `git tag vX.Y.Z` → 커밋과 태그 푸시. 문서만 바뀌면 버전 안 올림.
