...
마지막 갱신: 2026-09-25 / 현재 버전: **v0.38.1**(조플린은 0.38.0) (GitHub 태그·릴리스 완료) / 작업 폴더: `C:\claude program\obsidian-clipper-plus`
...
**다음 후보**
0. **아카 시간 초과**(2026-09-25): 고해상도 그림 여러 장인 글에서 2분 제한을 넘김. 그날 테스트로 요청을 많이 보내 429 제한에 걸린 상태였을 가능성이 큼 — 제한이 풀린 뒤 다시 확인. 계속 나면 이미지 동시 요청 수 제한이나 재시도 대기 시간 조정 검토.
0. **표 칸 안의 목록**(사용자가 "귀찮으니 그냥 가자"고 보류): 칸 안에서 `-   항목`이 글자로 보임 → 공유 코드 `jcpTableCellOneLine`에서 `• 항목`으로 바꾸면 깔끔. 합친 칸(colspan/rowspan)은 마크다운으로 표현 불가.
0. **동영상(움짤 mp4)**: 루리웹 등의 `<video>`는 원격 주소로만 남음 — 옵시디언에서는 첨부 파일로 저장하는 것 검토.
...
3. **옵시디언에서 눈으로 확인**: ~~Full Page 노트의 `[[x.html]]` 링크~~(사용자가 옵시디언에서 눌러 보고 글이 보이는 것 확인, 2026-09-25), 표 밖으로 빼낸 `![[파일|너비]]`가 제대로 보이는지는 사용자가 앱에서 확인해 줘야 함.
